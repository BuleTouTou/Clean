...
def main():
    ...
    sheet.append(["连心园", "朝阳", "常营", "3号楼", "1单元", "101", 500, "0013800000000", "0260616013", "260616013", "S级"])
    ...
    try:
        ...
        assert exported["report"]["任务开始时间"].endswith("Z")
        assert exported["report"]["完成时间"].endswith("Z")
        assert all(item["操作时间"].endswith("Z") for item in exported["report"]["审计记录"])
