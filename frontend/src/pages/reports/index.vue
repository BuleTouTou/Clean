...
const reportColumns = [
  ...
  },
  {
    title: "记录",
    key: "output_rows",
    render: (row: ReportItem) => `${row.output_rows} / ${row.input_rows}`,
  },
  {
    title: "阻断异常",
    key: "blocking_count",
  ...
  },
  {
    title: "完成时间（北京时间）",
    key: "completed_at",
    render: (row: ReportItem) => formatBeijingTime(row.completed_at),
  },
  {
    title: "操作",
    key: "actions",
      ...
      h(
        ...
        { size: 8 },
        {
          default: () => [
            h(
              NButton,
              { size: "small", tertiary: true, onClick: () => openRules(row) },
              { default: () => "查看规则" },
            ),
            h(
              NButton,
...
<template>
  <div class="mb-8">
    <div>
      <div
        class="text-sm font-semibold uppercase tracking-widest text-tsinghua"
      >
        历史报告
      </div>
    ...
    <n-spin :show="reportQuery.isLoading.value">
      ...
      <div class="mt-5 flex flex-wrap items-center justify-end gap-3">
        <n-select
          ...
          size="small"
          :options="
            [10, 20, 30, 50].map((value) => ({ label: `${value}条/页`, value }))
          "
        ...
        <n-pagination
          ...
          :item-count="reportQuery.data.value?.total ?? 0"
        />
      </div>
    </n-spin>
  </n-card>
